...
func (r fakeMySQLReleaseReminderRow) Scan(dest ...any) error {
	...
	*dest[3].(*string) = r.reminder.HostArchitecture
	*dest[4].(*string) = r.reminder.HostCreatedAt
	*dest[5].(*string) = r.reminder.ReleaseDueAt
	*dest[6].(*string) = r.reminder.OwnerEmail
	*dest[7].(*string) = r.reminder.OwnerName
	*dest[8].(*string) = r.reminder.LastExtendedByEmail
	*dest[9].(*string) = r.reminder.LastExtendedByName
	*dest[10].(*string) = r.reminder.LastExtendedAt
	*dest[11].(*string) = r.reminder.LastNotifiedAt
	*dest[12].(*string) = r.reminder.ReleasedAt
	*dest[13].(*string) = r.reminder.Status
	*dest[14].(*bool) = r.reminder.AutoReleaseEnabled
	*dest[15].(*string) = r.reminder.AutoReleaseAt
	*dest[16].(*string) = r.reminder.AutoReleaseStartedAt
	*dest[17].(*string) = r.reminder.AutoReleaseLastAttemptAt
	*dest[18].(*string) = r.reminder.AutoReleaseAcceptedAt
	*dest[19].(*string) = r.reminder.AutoReleaseStalledNotifyClaimedAt
	*dest[20].(*string) = r.reminder.AutoReleaseStalledNotifiedAt
	*dest[21].(*string) = r.reminder.AutoReleaseNotifiedAt
	*dest[22].(*int) = r.reminder.AutoReleaseAttempts
	*dest[23].(*string) = r.reminder.AutoReleaseLastError
	*dest[24].(*string) = r.reminder.AutoReleaseState
	*dest[25].(*string) = r.reminder.CreatedAt
	*dest[26].(*string) = r.reminder.UpdatedAt
	return nil
}
...
func (tx *fakeMySQLReleaseReminderTransaction) Exec(query string, args ...any) error {
	...
	if query == mysqlOperationEventInsertQuery {
		tx.eventExecArgs = append([]any(nil), args...)
		if tx.eventExecErr != nil {
	...
	case mysqlReleaseReminderInsertQuery:
		...
		tx.written = ReleaseReminder{
			...
			HostArchitecture:                  args[3].(string),
			HostCreatedAt:                     args[4].(string),
			ReleaseDueAt:                      args[5].(string),
			OwnerEmail:                        args[6].(string),
			OwnerName:                         args[7].(string),
			LastExtendedByEmail:               args[8].(string),
			LastExtendedByName:                args[9].(string),
			LastExtendedAt:                    args[10].(string),
			LastNotifiedAt:                    args[11].(string),
			ReleasedAt:                        args[12].(string),
			Status:                            args[13].(string),
			AutoReleaseEnabled:                args[14].(bool),
			AutoReleaseAt:                     args[15].(string),
			AutoReleaseStartedAt:              args[16].(string),
			AutoReleaseLastAttemptAt:          args[17].(string),
			AutoReleaseAcceptedAt:             args[18].(string),
			AutoReleaseStalledNotifyClaimedAt: args[19].(string),
			AutoReleaseStalledNotifiedAt:      args[20].(string),
			AutoReleaseNotifiedAt:             args[21].(string),
			AutoReleaseAttempts:               args[22].(int),
			AutoReleaseLastError:              args[23].(string),
			AutoReleaseState:                  args[24].(string),
			CreatedAt:                         args[25].(string),
			UpdatedAt:                         args[26].(string),
		}
	case mysqlReleaseReminderUpdateQuery:
		...
		tx.written = ReleaseReminder{
			ProfileName:                       args[25].(string),
			AppleEmail:                        args[0].(string),
			HostID:                            args[1].(string),
			HostArchitecture:                  args[2].(string),
			...
			AutoReleaseAcceptedAt:             args[17].(string),
			AutoReleaseStalledNotifyClaimedAt: args[18].(string),
			AutoReleaseStalledNotifiedAt:      args[19].(string),
			AutoReleaseNotifiedAt:             args[20].(string),
			AutoReleaseAttempts:               args[21].(int),
			AutoReleaseLastError:              args[22].(string),
			AutoReleaseState:                  args[23].(string),
			UpdatedAt:                         args[24].(string),
		}
...
func TestMySQLReleaseReminderAutoReleaseSchemaMigrations(t *testing.T) {
	wantColumns := []string{
		...
		"auto_release_accepted_at",
		"auto_release_stalled_notified_at",
...
func TestMySQLOperationEventSchemaMigrations(t *testing.T) {
	...
	for _, column := range []string{
		"member_email", "member_name", "request_id", "job_id", "cycle_id", "attempt", "session_id_hash", "source", "phase",
...
func TestMySQLSaveReleaseReminderInsertRoundTripsThroughScan(t *testing.T) {
	want := ReleaseReminder{
		...
		AutoReleaseAcceptedAt:        "2026-07-02T08:02:15Z",
		AutoReleaseStalledNotifiedAt: "2026-07-02T08:02:30Z",
...
func TestMySQLMemberStoreQueryEventsUsesDescendingKeysetPagination(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{
		...
		"request_id", "job_id", "cycle_id", "attempt", "session_id_hash", "source", "phase", "target_member_email", "error_code",
		"duration_ms", "confirmed", "status", "message", "created_at",
	}).
		AddRow("event-049", "aws.open", "iossupport-usw2", "apple@example.com", "member-1", "actor@example.com", "Actor", "request-1", "job-1", "arc-1", 3, "sha256:session", "web", "ready", "", "", int64(123), true, "success", "", "2026-07-30T09:59:00Z").
		AddRow("event-048", "aws.open", "iossupport-usw2", "apple@example.com", "member-1", "actor@example.com", "Actor", "request-1", "job-1", "arc-1", 2, "sha256:session", "web", "ready", "", "", int64(124), true, "success", "", "2026-07-30T09:58:00Z").
		AddRow("event-047", "aws.open", "iossupport-usw2", "apple@example.com", "member-1", "actor@example.com", "Actor", "request-1", "job-1", "arc-1", 1, "sha256:session", "web", "ready", "", "", int64(125), true, "success", "", "2026-07-30T09:57:00Z")
	...
	if len(page.Events) != 2 || page.Events[0].ID != "event-049" || page.Events[0].CycleID != "arc-1" || page.Events[0].Attempt != 3 || page.NextCursor == "" {
...
func TestMySQLMemberStoreRecordEventAppendsDirectly(t *testing.T) {
	...
	event := OperationEvent{
		...
		RequestID: " request-1 ", JobID: " job-1 ", CycleID: " arc-cycle-1 ", Attempt: 3, Source: " web ", Phase: " ready ",
	...
	mock.ExpectExec(regexp.QuoteMeta(mysqlOperationEventInsertQuery)).
		WithArgs(
			...
			"actor@example.com", "Actor", "request-1", "job-1", "arc-cycle-1", event.Attempt, "sha256:session-1", "web", "ready",
...
func TestMySQLMemberStoreRecordEventDuplicateExplicitIDIsIdempotent(t *testing.T) {
	...
	mock.ExpectExec(regexp.QuoteMeta(mysqlOperationEventInsertQuery)).
		WithArgs(
			event.ID, event.Action, event.Profile, "", "", "", "", "", "", "", 0, "", "", "", "", "",
...
func TestMySQLMemberStoreDirectAppendsGenerateDistinctIDs(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		...
		mock.ExpectExec(regexp.QuoteMeta(mysqlOperationEventInsertQuery)).
			WithArgs(
				idArgument, "test", "iossupport-usw2", "", "", "", "", "", "", "", 0, "", "", "", "", "",
...
func TestMySQLUpdateReleaseReminderSelectScanAndWriteRoundTrip(t *testing.T) {
	current := ReleaseReminder{
		...
		AutoReleaseLastAttemptAt:     "2026-07-02T08:02:00Z",
		AutoReleaseAcceptedAt:        "2026-07-02T08:02:15Z",
		AutoReleaseStalledNotifiedAt: "2026-07-02T08:02:20Z",
	...
	if len(tx.execArgs) != 26 {
		t.Fatalf("UPDATE arg count = %d, want 26", len(tx.execArgs))
	}
...
func TestMySQLUpdateReleaseReminderAndRecordEventRollsBackEventFailure(t *testing.T) {
	...
	_, err := updateReleaseReminderAndRecordEventInMySQLTransaction(tx, current.ProfileName, time.Date(2026, 7, 2, 8, 3, 0, 0, time.UTC), func(reminder ReleaseReminder) (ReleaseReminder, error) {
		...
		return reminder, nil
	}, OperationEvent{Action: "release-reminder.auto-release.enabled", MemberID: "member-1", MemberEmail: "admin@example.com", MemberName: "Admin", Attempt: 2, Status: "success"})
	if !errors.Is(err, wantErr) {
	...
	if len(tx.eventExecArgs) != 21 || tx.eventExecArgs[10] != 2 {
		t.Fatalf("event insert args = %#v, want attempt 2 at index 10", tx.eventExecArgs)
	}
}
