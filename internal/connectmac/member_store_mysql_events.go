...
const mysqlOperationEventSelectColumns = `id, action, profile, COALESCE(apple_email, ''), COALESCE(member_id, ''), COALESCE(member_email, ''), COALESCE(member_name, ''), COALESCE(request_id, ''), COALESCE(job_id, ''), COALESCE(source, ''), COALESCE(phase, ''), COALESCE(target_member_email, ''), COALESCE(error_code, ''), duration_ms, confirmed, status, COALESCE(message, ''), created_at`

const mysqlOperationEventInsertQuery = `INSERT INTO cm_events (id, action, profile, apple_email, member_id, member_email, member_name, request_id, job_id, source, phase, target_member_email, error_code, duration_ms, confirmed, status, message, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
...
func operationEventInsertArgs(event OperationEvent) []any {
	return []any{
		...
		event.Source, event.Phase, event.TargetMemberEmail, event.ErrorCode,
